...
class List extends componentsCore.Box {
  ...
  fuzzyFind(search, back) {
    ...
    if (!back) {
      for (i = start; i < this.ritems.length; i++) {
        if (test(utilHelpers.helpers.cleanTags(this.ritems[i]))) return i;
      }
  ...
  getItemIndex(child) {
    if (typeof child === 'number') {
      ...
      for (i = 0; i < this.ritems.length; i++) {
        if (utilHelpers.helpers.cleanTags(this.ritems[i]) === child) {
          return i;
        }
      }
  ...
  select(index) {
    ...
    this.selected = index;
    this.value = utilHelpers.helpers.cleanTags(this.ritems[this.selected]);
  ...
  pick(label, callback) {
    ...
    this.once('action', function (el, selected) {
      ...
      return callback(null, utilHelpers.helpers.cleanTags(self.ritems[selected]));
    });
  }
...
class Listbar extends componentsCore.Box {
  ...
  appendItem(item, callback) {
    ...
    const t = utilHelpers.helpers.generateTags(this.style.prefix || {
      fg: 'lightblack'
    });
    ...
    const options = {
      ...
      tags: true,
      mouse: true,
      style: utilHelpers.helpers.merge({}, this.style.item),
      noOverflow: true
    };
    ...
    var el = new componentsCore.Box(options);
    this._[cmd.text] = el;
    cmd.element = el;
    el._.cmd = cmd;
