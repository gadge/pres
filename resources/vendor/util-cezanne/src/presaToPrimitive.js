...
import { presaToSgra }   from './cezanne.js'
...
export const presaToPrimitive = function (hint) {
  let [ e, f, b ] = this
  const ec        = this.effectCollection
  if (hint === NUM) { return parseInt(n.sty(e) + n(f) + n(b), 16) }
  if (hint === STR) { return proj.render(e, ec) + Fluo.int(t(f), t) + Fluo.int(t(b), b) }
  if (hint === DEF) { return `[${ proj.render(e, ec) },${ Fluo.int(t(f), t) },${ Fluo.int(t(b), b) }]` }
  return presaToSgra(this) + (this.ch ?? '+') + CSI + SGR // throw new Error()
}