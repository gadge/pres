    ...
    if (options.keys) {
      this.on(KEYPRESS, (ch, key) => {
        ...
        if (options.vi && key.name === 'l' && key.shift) {
          ...
          self.down(self.childBase + Math.min(self.height - self.intH, self.items.length) - self.selected);
          self.screen.render();
          return;
        }
        ...
        if (options.vi && (key.ch === '/' || key.ch === '?')) {
          ...
          return self.options.search((err, value) => {
            ...
            if (typeof err === STR || typeof err === FUN || typeof err === NUM || (_err = err) != null && _err.test) {
              value = err;
              err = null;
            }
...
export { DataTable, List, ListBar, ListTable, Table, Tree, dataTable, list, listBar, listTable, table, tree };
