...
class List extends componentsCore.Box {
  ...
  constructor(options = {}) {
    ...
    if (options.mouse) {
      ...
      this.on(enumEvents.ELEMENT_WHEELDOWN, () => {
        self.select(self.selected + 2);
        self.screen.render();
      });
      this.on(enumEvents.ELEMENT_WHEELUP, () => {
        self.select(self.selected - 2);
        self.screen.render();
      });
    }
    ...
    if (options.keys) {
      this.on(enumEvents.KEYPRESS, (ch, key) => {
        if (key.name === enumKeyNames.UP || options.vi && key.name === 'k') {
          self.up();
          self.screen.render();
          return;
        }

        if (key.name === enumKeyNames.DOWN || options.vi && key.name === 'j') {
          self.down();
          self.screen.render();
          return;
        }

        if (key.name === enumKeyNames.ENTER || options.vi && key.name === 'l' && !key.shift) {
          self.enterSelected();
          return;
        }

        if (key.name === enumKeyNames.ESCAPE || options.vi && key.name === 'q') {
          self.cancelSelected();
          return;
        }
        ...
        if (options.vi && (key.ch === '/' || key.ch === '?')) {
          if (typeof self.options.search !== enumDataTypes.FUN) return;
          return self.options.search((err, value) => {
            var _err;

            if (typeof err === enumDataTypes.STR || typeof err === enumDataTypes.FUN || typeof err === enumDataTypes.NUM || (_err = err) !== null && _err !== void 0 && _err.test) {
              value = err;
              err = null;
            }
  ...
  createItem(content) {
    ...
    const options = {
      screen: this.screen,
      content: content,
      align: this.align || enumKeyNames.LEFT,
      top: 0,
      left: 0,
    ...
    EFFECT_COLLECTION.forEach(name => {
      options[name] = () => {
        ...
        if (typeof attr === enumDataTypes.FUN) attr = attr(item);
        return attr;
      };
    });
    if (this.style.transparent) options.transparent = true;
    const item = new componentsCore.Box(options);

    if (this.mouse) {
      item.on(enumEvents.CLICK, () => {
        self.focus();
  ...
  appendItem(content) {
    content = typeof content === enumDataTypes.STR ? content : content.getContent();
  ...
  fuzzyFind(search, back) {
    ...
    let i;
    if (typeof search === enumDataTypes.NUM) search += '';
    if (search && search[0] === '/' && search[search.length - 1] === '/') try {
      search = new RegExp(search.slice(1, -1));
    } catch (e) {}
    const test = typeof search === enumDataTypes.STR ? item => !!~item.indexOf(search) : search.test ? search.test.bind(search) : search;

    if (typeof test !== enumDataTypes.FUN) {
    ...
    if (!back) {
      for (i = start; i < this.ritems.length; i++) if (test(helpers__namespace.cleanTags(this.ritems[i]))) return i;

      for (i = 0; i < start; i++) if (test(helpers__namespace.cleanTags(this.ritems[i]))) return i;
    } else {
      for (i = start; i >= 0; i--) if (test(helpers__namespace.cleanTags(this.ritems[i]))) return i;

      for (i = this.ritems.length - 1; i > start; i--) if (test(helpers__namespace.cleanTags(this.ritems[i]))) return i;
    }
  ...
  getItemIndex(child) {
    if (typeof child === enumDataTypes.NUM) {
      return child;
    } else if (typeof child === enumDataTypes.STR) {
      let i = this.ritems.indexOf(child);
      if (~i) return i;

      for (i = 0; i < this.ritems.length; i++) if (helpers__namespace.cleanTags(this.ritems[i]) === child) return i;
  ...
  select(index) {
    ...
    if (typeof index === enumDataTypes.OBJ) {
    ...
    this.selected = index;
    this.value = helpers__namespace.cleanTags(this.ritems[this.selected]);
    if (!this.sup) return;
  ...
  pick(label, callback) {
    ...
    this.screen.render();
    this.once(enumEvents.ACTION, (el, selected) => {
      if (label) self.removeLabel();
      self.screen.restoreFocus();
      self.hide();
      self.screen.render();
      if (!el) return callback();
      return callback(null, helpers__namespace.cleanTags(self.ritems[selected]));
    });
  }
...
class DataTable extends componentsCore.Box {
  constructor(options) {
    ...
    options.interactive = typeof options.interactive === enumDataTypes.UND ? true : options.interactive;
  ...
  render() {
    ...
    componentsCore.Box.prototype.render.call(this);
  }
  ...
  update(table) {
    const self = this;
    let head = vectorMapper.mapper(table.head ?? table.headers, String),
        rows = matrixMapper.mapper(table.rows ?? table.data, String);
    const presets$1 = [presets.FRESH, presets.METRO, presets.SUBTLE];
    const padTable = tablePadder.tablePadder({
      head,
      rows
    }, {
      ansi: true,
    ...
    if (presets$1) {
      const [alpha, beta, gamma] = presets$1;
      head = fluoVector.fluoVector.call(enumColorantModes.MUTATE_PIGMENT, padTable.head, [alpha, gamma ?? beta]);
      rows = fluoMatrix.fluoMatrix.call(enumColorantModes.MUTATE_PIGMENT, padTable.rows, fluo.COLUMNWISE, [alpha, beta]);
    }

    const space = self.options.columnSpacing ? enumChars.SP.repeat(self.options.columnSpacing) : enumChars.SP;
    ...
    const dataToString = row => {
      ...
      row.forEach((cell, i) => {
        const text = String(cell),
              ...
              strip = charsetAnsi.clearAnsi(text),
...
class ListBar extends componentsCore.Box {
  ...
  constructor(options = {}) {
    ...
    if (options.keys) {
      this.on(enumEvents.KEYPRESS, (ch, key) => {
        if (key.name === enumKeyNames.LEFT || options.vi && key.name === 'h' || key.shift && key.name === enumKeyNames.TAB) {
          self.moveLeft();
          self.screen.render(); // Stop propagation if we're in a form.

          if (key.name === enumKeyNames.TAB) return false;
          return void 0;
        }

        if (key.name === enumKeyNames.RIGHT || options.vi && key.name === 'l' || key.name === enumKeyNames.TAB) {
          self.moveRight();
          self.screen.render(); // Stop propagation if we're in a form.
        ...
        if (key.name === enumKeyNames.ENTER || key.name === enumKeyNames.RETURN || options.vi && key.name === 'k' && !key.shift) {
    ...
    if (options.autoCommandKeys) {
      this.onScreenEvent(enumEvents.KEYPRESS, ch => {
        if (/^[0-9]$/.test(ch)) {
          let i = +ch - 1;
          if (!~i) i = 9;
    ...
    this.on(enumEvents.FOCUS, () => self.select(self.selected));
    this.type = 'list-bar';
  }
  ...
  setItems(commands) {
    const self = this;
    if (!Array.isArray(commands)) commands = vectorMapper.mapper(Object.entries(commands), ([key, conf], i) => {
      if (typeof conf === enumDataTypes.FUN) conf = {
        callback: conf
      };
  ...
  parseItemConfig(item, callback) {
    ...
    const conf = typeof item === enumDataTypes.OBJ ? (item.prefix = item.prefix ?? String(items.length + 1), item) : typeof item === enumDataTypes.STR ? {
      prefix: String(items.length + 1),
      text: item,
      callback: callback
    } : typeof item === enumDataTypes.FUN ? {
      prefix: String(items.length + 1),
      text: item.name,
      callback: item
    } : {};
  ...
  appendItem(item, callback) {
    ...
    const prev = vector.last(this.items);
    const drawn = this.sup ? (prev ? prev.absL + prev.width : 0) + (!this.screen.autoPadding ? this.intL : 0) : 0;
    const tags = helpers__namespace.generateTags(this.style.prefix ?? {
      fg: 'light-black'
    });
    ...
    if (!this.screen.autoPadding) {
      ...
      if (typeof opts.top === enumDataTypes.NUM) opts.top += this.intT;
      if (typeof opts.left === enumDataTypes.NUM) opts.left += this.intL;
    }

    const box = this.data[conf.text] = conf.element = new componentsCore.Box(opts);
    ...
    for (const effect of EFFECT_COLLECTION) {
      opts.style[effect] = () => {
        ...
        return typeof attr === enumDataTypes.FUN ? attr(box) : attr;
      };
    }
    ...
    if (conf.callback && conf.keys) {
      this.screen.key(conf.keys, () => {
        self.emit(enumEvents.ACTION, box, self.selected);
        self.emit(enumEvents.SELECT, box, self.selected);
  ...
  select(offset) {
    if (typeof offset !== enumDataTypes.NUM) offset = this.items.indexOf(offset);
    ...
    this.emit(enumEvents.SELECT_ITEM, item, offset);
  }

  removeItem(item) {
    const index = typeof item === enumDataTypes.NUM ? item : this.items.indexOf(item);
...
class Table extends componentsCore.Box {
      ...
      row.forEach((cell, i) => {
        ...
        while (clen < width) {
          if (align === 'center') {
            cell = ' ' + cell + ' ';
            clen += 2;
          } else if (align === enumKeyNames.LEFT) {
            cell = cell + ' ';
            clen += 1;
          } else if (align === enumKeyNames.RIGHT) {
            cell = ' ' + cell;
            clen += 1;
          }
        }
  ...
  render() {
    ...
    const normAttr = utilSgrAttr.styleToAttr(this.style),
          headAttr = utilSgrAttr.styleToAttr(this.style.header),
          cellAttr = utilSgrAttr.styleToAttr(this.style.cell),
...
class ListTable extends List {
  ...
  constructor(options = {}) {
    ...
    this._header = new componentsCore.Box({
      sup: this,
    ...
    this.setData(options.rows || options.data);
    this.on(enumEvents.ATTACH, () => self.setData(self.rows));
    this.on(enumEvents.RESIZE, () => {
  ...
  render() {
    ...
    const lines = this.screen.lines,
          ...
          yLo = coords.yLo;
    let rx, ry;
    const borderAttr = utilSgrAttr.styleToAttr(this.style.border);
...
class Tree extends componentsCore.Box {
  constructor(options) {
  ...
  walk(node, treeDepth) {
    ...
    if (node.sub && node.extended) {
      let i = 0;
      if (typeof node.sub === enumDataTypes.FUN) node.subContent = node.sub(node);
      ...
      for (let unit in node.subContent) {
        ...
        if (typeof unit.sub === enumDataTypes.FUN) unit.subContent = unit.sub(unit);else unit.subContent = unit.sub;
