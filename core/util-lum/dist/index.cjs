...
var projectorFactory = require('@palett/projector-factory')
...
const NAC$1 = 1 << 24
const GREY$1 = convert.rgbToInt([ 127, 127, 127 ])
...
const proj = projectorFactory.ProjectorFactory.fromHEX({
  min: 0,
  max: 32
}, presets.ATLAS)
const DASH2 = fluo.Fluo.hex('--', cards.Grey.darken_3)
const DASH6 = fluo.Fluo.hex('------', cards.Grey.darken_3)
...
const lumToPrimitive = function (hint) {
  ...
  if (hint === enumDataTypes.STR) {
    return proj.render(m, mode) + fluo.Fluo.int(t(f), t) + fluo.Fluo.int(t(b), b)
  }
  ...
  return lumToSgr(this) + ( this.ch ?? '+' ) + enumControlChars.CSI + enumCsiCodes.SGR // throw new Error()
}

const NAC = null
const GREY = convert.rgbToInt([ 127, 127, 127 ])
class Lum {
    ...
    for (let i = 0, hi = vec.length, c; i < hi; i++) {
      ...
      if (c === 0) {
        ...
        if (+vec[i + 1] === 5) {
          ...
          ++i, f = vec[++i] + enumChars.SC + vec[++i] + enumChars.SC + vec[++i]
        }
      }
      else if (c === 39) {
        f = norm[1]
        ...
        if (+vec[i + 1] === 5) {
          ...
          ++i, b = vec[++i] + enumChars.SC + vec[++i] + enumChars.SC + vec[++i]
        }
      }
      else if (c === 49) {
        b = norm[2]
...
const nameToColor = name => {
  ...
  if (!nullish.nullish(i)) return isNaN(i) ? NAC$2 : COLORS_4BITS[i] // 按位取反

  if (LIGHT.test(name)) i = COLOR_CODES[name.replace(LIGHT, '')]
  if (!nullish.nullish(i)) return isNaN(i) ? NAC$2 : COLORS_4BITS[i === 8 ? i - 1 : i + 8]
  return null
}
const convHex = hex => hex[0] === '#' ? convert.hexToInt(hex) : null
const toInt = color => {
  const t = typeof color
  color = t === enumDataTypes.NUM ? color : t === enumDataTypes.STR ? convHex(color) ?? nameToColor(color) ?? NAC$2 : Array.isArray(color) ? convert.rgbToInt(color) : NAC$2
  return color
}
function styleToLum(style = {}, fore, back) {
  if (nullish.nullish(fore) && nullish.nullish(back)) {
    fore = style.fore || style.fg, back = style.back || style.bg
  }

  if (typeof fore === enumDataTypes.FUN) fore = fore(this)
  if (typeof back === enumDataTypes.FUN) back = back(this)
  return new Lum(utilSgrMode.styleToMode(style), toInt(fore), toInt(back))
}
/**
 *
 * @param {string} sgr
...
function lumToSgr(lum, tputColors) {
  ...
  if (!nullish.nullish(b)) {
    tx += '48;2;' + ( b >> 16 & 0xFF ) + ';' + ( b >> 8 & 0xFF ) + ';' + ( b & 0xFF ) + ';'
  }

  if (!nullish.nullish(f)) {
    tx += '38;2;' + ( f >> 16 & 0xFF ) + ';' + ( f >> 8 & 0xFF ) + ';' + ( f & 0xFF ) + ';'
  }

  if (tx[tx.length - 1] === ';') tx = tx.slice(0, -1)
  return enumControlChars.CSI + tx + enumCsiCodes.SGR
}

exports.COLORS_4BITS = COLORS_4BITS
exports.COLOR_CODES = COLOR_CODES
exports.Lum = Lum
exports.NAC = NAC$2
exports.blend = blend
exports.lumToSgr = lumToSgr
exports.mixColors = mixColors
exports.nameToColor = nameToColor
exports.sgrToLum = sgrToLum
exports.styleToLum = styleToLum
exports.toInt = toInt
