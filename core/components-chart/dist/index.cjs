...
var x256__default = /*#__PURE__*/_interopDefaultLegacy(x256);
var sparkline__default = /*#__PURE__*/_interopDefaultLegacy(sparkline);
...
function getFgCode(color) {
  ...
  if (typeof color == 'string' && color !== 'normal') {
    ...
    return '\x1b[38;5;' + x256__default["default"](color[0], color[1], color[2]) + 'm';
  } // Number
...
function getBgCode(color) {
  ...
  if (typeof color == 'string' && color !== 'normal') {
    ...
    return '\x1b[48;5;' + x256__default["default"](color[0], color[1], color[2]) + 'm';
  } // Number
  else if (typeof color == 'number') {
...
class BarChart extends componentsLayout.Canvas {
  constructor(options = {}) {
  ...
  setData(series) {
    ...
    const max = Math.max(vectorIndicator.maxBy(values), this.options.maxHeight);
    const {
      barY,
      labelY,
      valueY
    ...
    for (let i = 0, x = this.options.xOffset; i < values.length; i++, x += bars.spacing) {
      ...
      if (values[i] <= 0) {
        ...
        const height = math.round(barY * (values[i] / max));
...
class DonutChart extends componentsLayout.Canvas {
  constructor(options = {}) {
...
class Gauge extends componentsLayout.Canvas {
  constructor(options = {}) {
  ...
  setData(data) {
    return data !== null && data !== void 0 && data.length ? this.setStack(data) : typeof data === enumDataTypes.NUM ? this.setPercent(data) : void 0;
  }
...
class GaugeList extends componentsLayout.Canvas {
  constructor(options = {}) {
...
class LineChart extends componentsLayout.Canvas {
  constructor(options = {}) {
    if (nullish.nullish(options.labelStep)) options.labelStep = options.showNthLabel ?? 1;
    const style = options.style ?? (options.style = {});
    if (nullish.nullish(style.line)) style.line = 'yellow';
    if (nullish.nullish(style.text)) style.text = 'green';
    if (nullish.nullish(style.baseline)) style.baseline = 'black';
    if (nullish.nullish(options.legend)) options.legend = {};
    const padds = options.padds = Padds.build(options.padds ?? options);
    const ticks = options.ticks = utilChartTicks.Ticks.build(options.ticks ?? options);
  ...
  labelWidth(labels) {
    return vectorIndicator.maxBy(labels, x => x === null || x === void 0 ? void 0 : x.length) ?? 0;
  }
  ...
  drawLegend(seriesCollection) {
    ...
    this.legend = new componentsCore.Box({
    ...
    for (const series of seriesCollection) {
      ...
      const color = utilByteColors.toByte(style.line || this.options.style.line);
...
class Sparkline extends componentsCore.Box {
  constructor(options = {}) {
  ...
  setData(titles, datasets) {
    this.update(vectorZipper.zipper(datasets, titles, (vec, title) => (vec.title = title, vec)));
  }

  update(titledVectors) {
    const fg = this.options.style.titleFg;
    let content = RN;
    vectorMapper.iterate(titledVectors, titledVector => {
      content += '{bold}{' + fg + '-fg}' + titledVector.title + ':{/' + fg + '-fg}{/bold}' + RN;
      content += sparkline__default["default"](titledVector.slice(0, this.width - 2)) + RN + RN;
...
class StackedBarChart extends componentsLayout.Canvas {
  constructor(options = {}) {
  ...
  renderBarSection(x, y, data, curBarSummedValue, currentBarHeight, availableBarHeight, bg) {
    ...
    if (currStackHeight > 0) {
      ...
      if (this.options.showText) {
        const str = utils__namespace.abbrNumber(data.toString());
  ...
  addLegend(bars, x) {
    ...
    self.legend = new componentsCore.Box({
    ...
    for (let i = 0; i < bars.stackedCategory.length; i++) {
      const color = utils__namespace.getColorCode(self.options.barBgColor[i]);
