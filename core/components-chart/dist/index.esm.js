...
import sparkline$1 from 'sparkline';
...
class Labels {
  ...
  get list() {
    return _classPrivateFieldLooseBase(this, _list)[_list];
  }

  set list(value) {
    _classPrivateFieldLooseBase(this, _list)[_list] = value;
  }

  get length() {
    return _classPrivateFieldLooseBase(this, _list)[_list].length;
  }

  get labelWidth() {
    return maxBy(this.list, x => x == null ? void 0 : x.length) ?? 0;
  }
...
class Bars {
  constructor(options) {
    ...
    this.preset = {
      fore: options.barFgColor ?? ((_options$preset = options.preset) == null ? void 0 : _options$preset.fore) ?? 'white',
      back: options.barBgColor ?? ((_options$preset2 = options.preset) == null ? void 0 : _options$preset2.back) ?? 'blue',
      label: options.labelColor ?? ((_options$preset3 = options.preset) == null ? void 0 : _options$preset3.label) ?? 'white'
    };
  }
  ...
  setData(series) {
    ...
    for (let i = 0, x = this.options.xOffset; i < values.length; i++, x += bars.spacing) {
      ...
      if (values[i] <= 0) {
        ...
        context.strokeStyle = (_bars$preset = bars.preset) == null ? void 0 : _bars$preset.back;
        const height = round(barY * (values[i] / max));
        context.fillRect(x, barY - height + 1, bars.width, height);
      } // draw values


      context.fillStyle = (_bars$preset2 = bars.preset) == null ? void 0 : _bars$preset2.fore;
      if (this.options.showText) context.fillText(values[i].toString(), x + 1, valueY);
      context.strokeStyle = 'normal'; // draw labels

      context.fillStyle = (_bars$preset3 = bars.preset) == null ? void 0 : _bars$preset3.label;
...
class Gauge extends Canvas {
  ...
  setData(data) {
    return data != null && data.length ? this.setStack(data) : typeof data === NUM ? this.setPercent(data) : void 0;
  }
...
class LineChart extends Canvas {
  ...
  labelWidth(labels) {
    return maxBy(labels, x => x == null ? void 0 : x.length) ?? 0;
  }
  ...
  setData(seriesCollection) {
    ...
    for (let i = ticks.min; i < ticks.max; i += ticks.step) context.fillText(ticks.formatTick(i), padds.relativeX, this.coordY(i)); // Draw y-value series collection
...
class Sparkline extends Box {
  ...
  setData(titles, datasets) {
    ...
    for (let i = 0; i < titles.length; i++) {
      ...
      res += sparkline$1(datasets[i].slice(0, this.width - 2)) + '\r\n\r\n';
    }
...
export { BarChart, DonutChart, Gauge, GaugeList, LineChart, Sparkline, StackedBarChart, barChart, donutChart, gauge, gaugeList, lineChart, sparkline, stackedBarChart };
