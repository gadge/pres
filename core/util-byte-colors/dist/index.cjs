...
function hexToWeb(hex) {
  const s = convert.hexToShort(hex)
...
const degrade = (byte, total) => {
  ...
  if (total <= 16 && 16 <= byte) return CACHE16[byte] ?? ( CACHE16[byte] = ( _ref = ( _ref2 = ( _byte = byte, byteToHex(_byte) ), convert.hexToHsl(_ref2) ), hslToBit4(_ref) ) )

  if (total <= 8) {
    var _ref3, _ref4, _byte2

    if (16 <= byte) return CACHE8[byte] ?? ( CACHE8[byte] = ( _ref3 = ( _ref4 = ( _byte2 = byte, byteToHex(_byte2) ), convert.hexToHsl(_ref4) ), hslToBit3(_ref3) ) )
...
const SHORT16 = vector.mapper(COLOR_HEXES_BIT4, convert.hexToShort)
function hexToBit4(hex) {
  for (let i = 0, short = convert.hexToShort(hex); i < SHORT16.length; i++) if (short === SHORT16[i]) return i
...
function toByte(color) {
  ...
  if (t === enumDataTypes.NUM) return color & 0x1ff
  if (t === enumDataTypes.STR) return hexToByte(color) ?? nameToByte(color) ?? 0x1ff
  if (Array.isArray(color)) return rgbToByte.apply(null, color)
  return 0x1ff
}

exports.COLOR_HEXES_BIT4 = COLOR_HEXES_BIT4
exports.COLOR_NAMES_BIT3 = COLOR_NAMES_BIT3
exports.COLOR_NAMES_BIT4 = COLOR_NAMES_BIT4
exports.WEB_SAFE_MATRIX_HEX = WEB_SAFE_MATRIX_HEX
exports.WEB_SAFE_MATRIX_INDEX = WEB_SAFE_MATRIX_INDEX
exports.byteToBackSgra = byteToBackSgra
exports.byteToCoord = byteToCoord
exports.byteToForeSgra = byteToForeSgra
exports.byteToGrey = byteToGrey
exports.byteToHex = byteToHex
exports.byteToWeb = byteToWeb
exports.coordToByte = coordToByte
exports.coordToWeb = coordToWeb
exports.degrade = degrade
exports.hexToByte = hexToByte
exports.hexToCoord = hexToCoord
exports.hexToWeb = hexToWeb
exports.rgbToByte = rgbToByte
exports.srgbToCoord = srgbToCoord
exports.srgbToWeb = srgbToWeb
