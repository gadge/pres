...
class EventEmitter {
  ...
  on(type, listener) {
    let curr = _classPrivateFieldLooseBase(this, _events)[_events][type];

    if (!curr) {
      _classPrivateFieldLooseBase(this, _events)[_events][type] = listener;
    } else if (typeof curr === FUN) {
      _classPrivateFieldLooseBase(this, _events)[_events][type] = [curr, listener];
    } else {
  ...
  off(type, listener) {
    const curr = _classPrivateFieldLooseBase(this, _events)[_events][type];
    ...
    for (let i = 0, hi = curr.length; i < hi; i++) {
      if (curr[i] === listener || curr[i].listener === listener) {
        curr.splice(i, 1);
        return void _classPrivateFieldLooseBase(this, _emit)[_emit](REMOVE_LISTENER, [type, listener]);
      }
    }
  }
  ...
  emit(type, ...args) {
    ...
    _classPrivateFieldLooseBase(this, _emit)[_emit](EVENT, slice(arguments));
