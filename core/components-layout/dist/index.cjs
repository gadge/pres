...
var utilDrawilleCanvas = require('@pres/util-drawille-canvas');
var math = require('@aryth/math');
var utilHelpers = require('@pres/util-helpers');

class Canvas extends componentsCore.Box {
  constructor(options = {}, canvasType) {
    if (!options.sku) options.sku = 'canvas';
    super(options);
    this.on(enumEvents.ATTACH, () => {
      this.context = new utilDrawilleCanvas.Context(this.canvW, this.canvH, canvasType);
...
class Grid {
  constructor(options) {
    ...
    this.margin = componentsCore.Cadre.build(options.margin);
    this.unit = {
      h: math.roundD4((100 - this.margin.vert) / this.rowNo),
      // roundD2(( 100 - this.margin * 2 ) / this.rowNo),
      w: math.roundD4((100 - this.margin.hori) / this.colNo),
  ...
  set(t, l, h, w, component, options) {
    ...
    let p = {};
    p = utilHelpers.assignDeep(p, options);
    p.top = options.t ?? math.roundD2(t * this.unit.h) + '%' + (this.margin.t ? '+' + this.margin.t : ''); // ( t * this.unit.h + this.margin ) + '%'

    p.left = options.l ?? math.roundD2(l * this.unit.w) + '%' + (this.margin.l ? '+' + this.margin.l : ''); // ( l * this.unit.w + this.margin ) + '%'

    p.height = options.h ?? math.roundD2(this.unit.h * h - SPACING) + '%'; // + '-' + this.margin.t

    p.width = options.w ?? math.roundD2(this.unit.w * w - SPACING) + '%'; // console.log('[coord]', `(${ p.top },${ p.left })`, '[size]', `(${ p.height },${ p.width })`)
...
class Page extends componentsCore.Box {
  elements = [];
  ...
  constructor(options) {
    ...
    this.hideBorder = options.hideBorder;
    this.unit = {
      h: math.roundD4(100 / this.rowNo),
      w: math.roundD4(100 / this.colNo),
      border: {
        type: 'line',
        fg: options.color ?? 'cyan'
      }
  ...
  add(t, l, h, w, component, options) {
    ...
    let p = {};
    p = utilHelpers.assignDeep(p, options);
    p.top = options.t ?? math.roundD2(t * this.unit.h) + '%';
    p.left = options.l ?? math.roundD2(l * this.unit.w) + '%';
    p.height = options.h ?? math.roundD2(this.unit.h * h) + '%'; // + '-' + this.margin.t

    p.width = options.w ?? math.roundD2(this.unit.w * w) + '%'; // console.log('[coord]', `(${ p.top },${ p.left })`, '[size]', `(${ p.height },${ p.width })`)
