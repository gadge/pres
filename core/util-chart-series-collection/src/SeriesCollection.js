...
export class SeriesCollection extends Array {
  constructor(length) {
    super(length)
  }
  static fromSeriesList(seriesList) {
    ...
    if (length === 0) { }
    if (length === 1) { seriesCollection[0] = seriesList[0] }
    if (length >= 2) { for (let i = 0; i < length; i++) seriesCollection[i] = seriesList[i] }
