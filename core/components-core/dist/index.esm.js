...
import { NEW_LISTENER, CLICK, MOUSE, MOUSEDOWN, MOUSEUP, MOUSEMOVE, MOUSEOVER, MOUSEOUT, MOUSEWHEEL, WHEELDOWN, WHEELUP, KEYPRESS, KEY, RESIZE, ATTACH, DETACH, MOVE, PARSED_CONTENT, PRERENDER, RENDER, HIDE, SHOW, SET_CONTENT, SCROLL, LOG, FOCUS, BLUR, WARNING, DESTROY, ELEMENT_CLICK, ELEMENT_MOUSEOVER, ELEMENT_MOUSEOUT, ELEMENT_MOUSEUP, ERROR, EXIT, DATA, TITLE, PASSTHROUGH } from '@pres/enum-events';
import { LEFT, TOP, CENTER, MIDDLE, BOTTOM, RIGHT, UP, DOWN } from '@pres/enum-key-names';
...
const EVENT_LIST_COLLECTION = [['hoverEffects', 'mouseover', 'mouseout', '_htemp'], ['focusEffects', 'focus', 'blur', '_ftemp']];

var _parsedContent = /*#__PURE__*/_classPrivateFieldLooseKey("parsedContent");

var _wrapContent = /*#__PURE__*/_classPrivateFieldLooseKey("wrapContent");

var _parseTags = /*#__PURE__*/_classPrivateFieldLooseKey("parseTags");

var _parseAttr = /*#__PURE__*/_classPrivateFieldLooseKey("parseAttr");
...
class Element extends Node {
  ...
  constructor(options = {}, lazy) {
    ...
    super(options);
    Object.defineProperty(this, _parseAttr, {
      value: _parseAttr2
    });
    Object.defineProperty(this, _parseTags, {
      value: _parseTags2
    });
    Object.defineProperty(this, _wrapContent, {
      value: _wrapContent2
    });
    Object.defineProperty(this, _parsedContent, {
      writable: true,
      value: void 0
    });
    this.type = 'element';
  ...
  config(options) {
    ...
    this.style = options.style ?? selectObject(options, ['fg', 'bg', 'bold', 'underline', 'blink', 'inverse', 'invisible', 'transparent']);
    ...
    this.ch = options.ch || ' ';
    const pos = this.pos = Detic.build(options.pos ?? (options.pos = selectObject(options, ['left', 'right', 'top', 'bottom', 'width', 'height'])));
  ...
  calcCoords(get, noScroll) {
    if (this.hidden) return; // if (this.sup._rendering) {
    //   get = true;
    // }
    ...
    if (node && !noScroll) {
      supPos = sup.prevPos; // The shrink option can cause a stack overflow
      // by calling calcCoords on the child again.
      // if (!get && !thisparent.shrink) {
      //   supPos = thisparent.calcCoords();
      // }

      if (!supPos) return; // TODO: Figure out how to fix base (and cbase to only
      // take into account the *sup's* padding.

      yLo -= supPos.base;
      yHi -= supPos.base;
      b = sup.border ? 1 : 0; // XXX
      ...
      if (yLo >= yHi) return; // Could allow overlapping stuff in scrolling elements
    ...
    if (this.noOverflow && this.sup.prevPos) {
      if (xLo < this.sup.prevPos.xLo + this.sup.intL) {
        xLo = this.sup.prevPos.xLo + this.sup.intL;
      }

      if (xHi > this.sup.prevPos.xHi - this.sup.intR) {
        xHi = this.sup.prevPos.xHi - this.sup.intR;
      }

      if (yLo < this.sup.prevPos.yLo + this.sup.intT) {
        yLo = this.sup.prevPos.yLo + this.sup.intT;
      }

      if (yHi > this.sup.prevPos.yHi - this.sup.intB) {
        yHi = this.sup.prevPos.yHi - this.sup.intB;
      }
    } // if (this.sup.prevPos) {
  ...
  calcShrinkBox(xLo, xHi, yLo, yHi, get) {
    ...
    for (i = 0; i < this.sub.length; i++) {
      el = this.sub[i];
      ret = el.calcCoords(get); // Or just (seemed to work, but probably not good):
  ...
  parseContent(noTags) {
    ...
    if (this.contLines == null || this.contLines.width !== width || this.contLines.content !== this.content) {
      ...
      if (!noTags) {
        content = _classPrivateFieldLooseBase(this, _parseTags)[_parseTags](content);
      }

      this.contLines = _classPrivateFieldLooseBase(this, _wrapContent)[_wrapContent](content, width); // console.log(`>> [{${ this.codename }}.contLines]`, this.contLines)

      this.contLines.width = width;
      this.contLines.content = this.content;
      this.contLines.attr = _classPrivateFieldLooseBase(this, _parseAttr)[_parseAttr](this.contLines);
      this.contLines.ci = [];
      this.contLines.reduce((total, line) => {
        this.contLines.ci.push(total);
        return total + line.length + 1;
      }, 0);
      _classPrivateFieldLooseBase(this, _parsedContent)[_parsedContent] = this.contLines.join(LF);
      this.emit(PARSED_CONTENT);
      return true;
    } // Need to calculate this every time because the default fg/bg may change.


    this.contLines.attr = _classPrivateFieldLooseBase(this, _parseAttr)[_parseAttr](this.contLines) || this.contLines.attr;
    return false;
  }
  ...
  render() {
    ...
    const content = _classPrivateFieldLooseBase(this, _parsedContent)[_parsedContent]; // if (this.codename === 'box.24') console.log('box.24', 'this.content', `[ ${ this.content } ]`)
    ...
    let ci = this.contLines.ci[coords.base],
        ...
        i;
    const bch = this.ch;
    if (coords.base >= this.contLines.ci.length) ci = _classPrivateFieldLooseBase(this, _parsedContent)[_parsedContent].length;
    this.prevPos = coords;

    if (((_this$border = this.border) == null ? void 0 : _this$border.type) === 'line') {
      ...
      for (let y = yLo + 1, line, cell; y < yHi - 1; y++) {
        ...
        if (cell = line[xLo]) {
          if (this.border.left) {
            ...
            if (!coords.negL) if (cell.at !== borderAttr || cell.ch !== ch) {
              cell.inject(borderAttr, ch);
              line.dirty = true;
            }
          } else {
        ...
        if (cell = line[xHi - 1]) {
          if (this.border.right) {
            ...
            if (!coords.negR) if (cell.at !== borderAttr || cell.ch !== ch) {
              cell.inject(borderAttr, ch);
              line.dirty = true;
            }
          } else {
  ...
  getContent() {
    ...
    return ((_this$contLines = this.contLines) == null ? void 0 : _this$contLines.fake.join(LF)) ?? '';
  }
  ...
  setLabel(options) {
    ...
    const reposition = () => {
      ...
      if (!self.screen.autoPadding) {
        self._label.relT = self.childBase || 0;
      }
    ...
    this.on(RESIZE, this._labelResize = () => nextTick$2(() => reposition()));
  }
...
function _wrapContent2(content, width) {
...
class Scroll {
  ...
  config(options) {
    console.log('>> [set scroll]', this == null ? void 0 : this.codename);
  ...
  scroll(offset, always) {
    ...
    this.childBase = this.childBase < 0 ? 0 : this.childBase > this.baseLimit ? this.baseLimit : this.childBase; // Find max "bottom" value for
    // content and descendant elements.
    // Scroll the content if necessary.

    if (this.childBase === base) return this.emit(SCROLL); // When scrolling text, we want to be able to handle SGR codes as well as line
    ...
    this.childBase = this.childBase < 0 ? 0 : this.childBase > this.baseLimit ? this.baseLimit : this.childBase; // Optimize scrolling with CSR + IL/DL.

    p = this.prevPos; // Only really need calcCoords() if we want
...
var _reduceColor = /*#__PURE__*/_classPrivateFieldLooseKey("reduceColor");

var _cursorAttr = /*#__PURE__*/_classPrivateFieldLooseKey("cursorAttr");

var _dockBorders = /*#__PURE__*/_classPrivateFieldLooseKey("dockBorders");

var _getAngle = /*#__PURE__*/_classPrivateFieldLooseKey("getAngle");

class Screen extends Node {
  constructor(options = {}) {
    options.sku = options.sku ?? 'screen';
    super(options, true);
    Object.defineProperty(this, _getAngle, {
      value: _getAngle2
    });
    Object.defineProperty(this, _dockBorders, {
      value: _dockBorders2
    });
    Object.defineProperty(this, _cursorAttr, {
      value: _cursorAttr2
    });
    Object.defineProperty(this, _reduceColor, {
      value: _reduceColor2
    });
    this.type = 'screen';
  ...
  screenshot(xLo, xHi, yLo, yHi, term) {
    ...
    for (let y = yLo, line; y < yHi; y++) {
      if (!(line = (term == null ? void 0 : term.lines[y]) ?? this.currLines[y])) break;
      let out = '',
          currAttr = this.dattr;
  ...
  render() {
    ...
    if (this.screen.dockBorders) {
      _classPrivateFieldLooseBase(this, _dockBorders)[_dockBorders]();
    }
    ...
    if ((_this$focused = this.focused) != null && _this$focused._updateCursor) {
    ...
    for (let y = start; y <= end; y++) {
      ...
      for (let x = 0, currCell, prevCell; x < currLine.length && (currCell = currLine[x]); x++) {
        ...
        if (cursor.artificial && !cursor._hidden && cursor._state && x === program.x && y === program.y) {
          const cursorAttr = _classPrivateFieldLooseBase(this, _cursorAttr)[_cursorAttr](this.cursor, nextCell.at);
        ...
        if (options.useBCE && ch === ' ' && (tput.bools.back_color_erase || (at & 0x1ff) === (this.dattr & 0x1ff)) && (at >> 18 & 8) === (this.dattr >> 18 & 8)) {
          clr = true;
          neq = false;
...
function _reduceColor2(color) {
  return degrade(color, this.tput.colors);
}

function _cursorAttr2(cursor, normAttr) {
  const {
    shape
  } = cursor;
...
function _dockBorders2() {
  ...
  for (let i = 0, y, line, cell; i < stops.length; i++) {
    ...
    for (let x = 0; x < this.width; x++) {
      ...
      if (ANGLES[cell.ch]) {
        cell.ch = _classPrivateFieldLooseBase(this, _getAngle)[_getAngle](lines, x, y);
        line.dirty = true;
      }
    }
  }
...
export { Box, Element, Layout, Line, Log, Screen, ScrollableBox, ScrollableText, Terminal, box, element, layout, line, log, node, screen, scrollableBox, scrollableText, terminal };
