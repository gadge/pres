...
var enumCoordInfos = require('@pres/enum-coord-infos');
...
var enumSgrAttrs = require('@pres/enum-sgr-attrs');
var enumKeyNames = require('@pres/enum-key-names');
var enumAngleTable = require('@pres/enum-angle-table');
...
var utilMorisot = require('@pres/util-morisot');
...
const SGR_ATTRS = [enumSgrAttrs.FORE, enumSgrAttrs.FG, enumSgrAttrs.BACK, enumSgrAttrs.BG, enumSgrAttrs.BOLD, enumSgrAttrs.ITALIC, enumSgrAttrs.UNDERLINE, enumSgrAttrs.BLINK, enumSgrAttrs.REVERSE, enumSgrAttrs.INVERSE, enumSgrAttrs.HIDE, enumSgrAttrs.INVISIBLE, enumSgrAttrs.TRANSPARENT];
const COORD_INFOS = [enumCoordInfos.LEFT, enumCoordInfos.RIGHT, enumCoordInfos.TOP, enumCoordInfos.BOTTOM, enumCoordInfos.WIDTH, enumCoordInfos.HEIGHT];
...
class Cadre {
  ...
  static build(o) {
    const t = typeof o;
    if (!o || t === enumDataTypes.NUM) return o = o ?? 0, new Cadre(o, o, o, o);
    if (t === enumDataTypes.OBJ) return new Cadre(o.t ?? o.top ?? 0, o.b ?? o.bottom ?? 0, o.l ?? o.left ?? 0, o.r ?? o.right ?? 0);
...
class Element extends componentsNode.Node {
  #parsedContent;
  type = 'element';
  /**
   * Element
  ...
  config(options) {
    ...
    this.style = options.style ?? objectSelect.select.call(SGR_ATTRS, options);
    this.hidden = options.hidden || false;
    this.fixed = options.fixed || false;
    this.align = options.align || enumCoordInfos.LEFT;
    this.valign = options.valign || enumCoordInfos.TOP;
    this.wrap = options.wrap !== false;
    this.shrink = options.shrink;
    this.inGrid = options.inGrid;
    this.fixed = options.fixed;
    this.ch = options.ch || ' ';
    const pos = this.pos = Detic.build(options.pos ?? (options.pos = objectSelect.select.call(COORD_INFOS, options)));
    const widthShrink = pos.width === enumCoordInfos.SHRINK,
          heightShrink = pos.height === enumCoordInfos.SHRINK;

    if (widthShrink || heightShrink) {
      if (widthShrink) pos.delete(enumCoordInfos.WIDTH); //pos.width = null // delete pos.width

      if (heightShrink) pos.delete(enumCoordInfos.HEIGHT); // pos.height = null // delete pos.height
    ...
    if (this.border) {
      if (typeof this.border === enumDataTypes.STR) this.border = {
        type: this.border
      };
      ...
      if (nullish.nullish(this.border.left)) this.border.left = true;
      if (nullish.nullish(this.border.top)) this.border.top = true;
      if (nullish.nullish(this.border.right)) this.border.right = true;
      if (nullish.nullish(this.border.bottom)) this.border.bottom = true;
    ...
    this.on(enumEvents.NEW_LISTENER, type => {
      // type = type.split(' ').slice(1).join(' ');
      if (type === enumEvents.CLICK || type === enumEvents.MOUSE || type === enumEvents.MOUSEDOWN || type === enumEvents.MOUSEUP || type === enumEvents.MOUSEMOVE || type === enumEvents.MOUSEOVER || type === enumEvents.MOUSEOUT || type === enumEvents.MOUSEWHEEL || type === enumEvents.WHEELDOWN || type === enumEvents.WHEELUP) {
        self.screen._listenMouse(self);
      } else if (type === enumEvents.KEYPRESS || type.indexOf(enumEvents.KEY + enumChars.SP) === 0) {
        self.screen._listenKeys(self);
      }
    });
    this.on(enumEvents.RESIZE, () => self.parseContent());
    this.on(enumEvents.ATTACH, () => self.parseContent());
    this.on(enumEvents.DETACH, () => delete self.prevPos);

    if (!nullish.nullish(options.hoverBg)) {
      if (nullish.nullish(options.hoverEffects)) options.hoverEffects = {};
  ...
  set width(val) {
    ...
    this.emit(enumEvents.RESIZE), this.clearPos();
  ...
  set absT(val) {
    if (typeof val === enumDataTypes.STR) if (val === enumCoordInfos.CENTER) {
    ...
    this.emit(enumEvents.MOVE), this.clearPos();
  ...
  calcT(get) {
    ...
    if (typeof top === enumDataTypes.STR) {
      if (top === enumCoordInfos.CENTER) {
        top = '50%';
      }

      top = this.inGrid ? this.scaler.scaleT(top, supPos.height) : scaler(top, supPos.height);
      if (this.pos.top === enumCoordInfos.CENTER) top -= this.calcH(get) / 2 | 0;
    }
    ...
    if (this.screen.autoPadding && (this.pos.top != null || this.pos.bottom == null) && this.pos.top !== enumCoordInfos.CENTER) {
      top += this.sup.intT;
    }
  ...
  calcL(get) {
    ...
    if (typeof left === enumDataTypes.STR) {
      if (left === enumCoordInfos.CENTER) {
        left = '50%';
      }

      left = this.inGrid ? this.scaler.scaleL(left, supPos.width) : scaler(left, supPos.width);

      if (this.pos.left === enumCoordInfos.CENTER) {
    ...
    if (this.screen.autoPadding && (this.pos.left != null || this.pos.right == null) && this.pos.left !== enumCoordInfos.CENTER) {
      left += this.sup.intL;
    }
  ...
  calcW(get) {
    ...
    if (typeof width === enumDataTypes.STR) {
      if (width === enumCoordInfos.HALF) {
        width = '50%';
      }
    ...
    if (width == null) {
      ...
      if (this.screen.autoPadding) {
        if ((this.pos.left != null || this.pos.right == null) && this.pos.left !== enumCoordInfos.CENTER) width -= this.sup.intL;
        width -= this.sup.intR;
      }
    }
  ...
  calcH(get) {
    ...
    if (typeof height === enumDataTypes.STR) {
      if (height === enumCoordInfos.HALF) {
        height = '50%';
      }
    ...
    if (height == null) {
      ...
      if (this.screen.autoPadding) {
        if ((this.pos.top != null || this.pos.bottom == null) && this.pos.top !== enumCoordInfos.CENTER) height -= this.sup.intT;
  ...
  calcPos() {
    ...
    if (!nullish.nullish(pos.absL)) return pos;
    const t = pos.t;
  ...
  calcShrink(xLo, xHi, yLo, yHi, get) {
    ...
    if (xHi < xll && this.pos.left === enumCoordInfos.CENTER) {
      xll = (xll - xHi) / 2 | 0;
      xLo += xll;
      xHi += xll;
    }

    if (yHi < yll && this.pos.top === enumCoordInfos.CENTER) {
      yll = (yll - yHi) / 2 | 0;
      yLo += yll;
      yHi += yll;
    }
  ...
  sattr(style, fg, bg) {
    return utilSgrAttr.styleToAttr(style, fg, bg);
  ...
  parseContent(noTags) {
    ...
    if (this.contLines == null || this.contLines.width !== width || this.contLines.content !== this.content) {
      ...
      if (this.screen.fullUnicode) {
        ...
        if (this.screen.program.isiTerm2) content = content.replace(unicode__namespace.chars.combining, '');
      } else {
      ...
      this.#parsedContent = this.contLines.join(enumControlChars.LF);
  ...
  #wrapContent(content, width) {
    ...
    let no = 0,
        ...
        lines,
        rest;
    lines = content.split(enumControlChars.LF);
    ...
    main: for (; no < lines.length; no++) {
      ...
      if (tags) {
        if ((matches = /^{(left|center|right)}/.exec(line)) && ([phrase, word] = matches)) {
          ...
          align = state = word !== enumCoordInfos.LEFT ? word : null;
        }
      ...
      while (line.length > width) {
        ...
        for (i = 0, total = 0; i < line.length; i++) {
          while (line[i] === enumControlChars.ESC) while (line[i] && line[i++] !== 'm') {}
      ...
      if (cap = /^{(\/?)([\w\-,;!#]*)}/.exec(text)) {
        ...
        if (slash) {
          if (!param) {
            ...
            if (!nullish.nullish(attr)) {
              state.pop();
              out += state.length ? program.parseAttr(vectorIndex.last(state)) : attr;
            } else {
              out += cap[0];
            }
          }
  ...
  #parseAttr(lines) {
    const normAttr = utilSgrAttr.styleToAttr(this.style);
    ...
    for (let j = 0, line; j < lines.length; j++) {
      ...
      for (let i = 0, matches, sgra; i < line.length; i++) {
        if (line[i] === enumControlChars.ESC) {
          if ((matches = REGEX_INIT_SGR.exec(line.slice(i))) && ([sgra] = matches)) {
            baseAttr = utilSgrAttr.sgraToAttr(sgra, baseAttr, normAttr);
            i += sgra.length - 1;
          }
        }
      }
  ...
  render() {
    ...
    normAttr = utilSgrAttr.styleToAttr(this.style); // console.log('>> [element.render] interim', dattr)
    ...
    if (this.padding.any || this.valign && this.valign !== enumCoordInfos.TOP) {
      if (this.style.transparent) {
        for (let y = Math.max(yLo, 0), line, cell; y < yHi; y++) {
          ...
          for (let x = Math.max(xLo, 0); x < xHi; x++) {
            ...
            cell.at = colors__namespace.blend(currAttr, cell.at);
    ...
    if (this.valign === enumCoordInfos.MIDDLE || this.valign === enumCoordInfos.BOTTOM) {
      visible = yHi - yLo;

      if (this.contLines.length < visible) {
        if (this.valign === enumCoordInfos.MIDDLE) {
          visible = visible / 2 | 0;
          visible -= this.contLines.length / 2 | 0;
        } else if (this.valign === enumCoordInfos.BOTTOM) {
    ...
    for (let y = yLo, line; y < yHi; y++) {
      ...
      for (let x = xLo, cell; x < xHi; x++) {
        ...
        while (ch === enumControlChars.ESC) {
          if ((matches = REGEX_INIT_SGR.exec(content.slice(ci - 1))) && ([sgra] = matches)) {
            ci += sgra.length - 1;
            currAttr = utilSgrAttr.sgraToAttr(sgra, currAttr, normAttr); // if (this.codename === 'box.25') console.log(
        ...
        if (ch === enumControlChars.TAB) ch = bch;

        if (ch === enumControlChars.LF) {
          // If we're on the first cell and we find a newline and the last cell
          // of the last line was not a newline, let's just treat this like the
          // newline was already "counted".
          if (x === xLo && y !== yLo && content[ci - 2] !== enumControlChars.LF) {
            x--;
            continue;
          ...
          for (; x < xHi; x++) {
            ...
            if (this.style.transparent) {
              cell.inject(colors__namespace.blend(currAttr, cell.at), content[ci] ? ch : null);
              line.dirty = true;
        ...
        if (this.style.transparent) {
          nextCell.inject(colors__namespace.blend(currAttr, nextCell.at), content[ci] ? ch : null);
          line.dirty = true;
        } else {
    ...
    if (this.scrollbar && yHi - yLo < i) {
      ...
      if (cell) {
        if (this.track) {
          ch = this.track.ch || ' ';
          currAttr = utilSgrAttr.styleToAttr(this.style.track, this.style.track.fg || this.style.fg, this.style.track.bg || this.style.bg);
          this.screen.fillRegion(currAttr, ch, x, x + 1, yLo, yHi);
        }

        ch = this.scrollbar.ch || ' ';
        currAttr = utilSgrAttr.styleToAttr(this.style.scrollbar, this.style.scrollbar.fg || this.style.fg, this.style.scrollbar.bg || this.style.bg);
    ...
    if (this.border) {
      borderAttr = utilSgrAttr.styleToAttr(this.style.border);
      let y = yLo;
      if (coord.negT) y = -1;
      let line = lines[y];
    ...
    if (this.shadow) {
      ...
      for (let y = Math.max(yLo + 1, 0), line; y < yHi + 1; y++) {
        ...
        for (let x = xHi, cell; x < xHi + 2; x++) {
          ...
          cell.at = colors__namespace.blend(cell.at);
          line.dirty = true;
        }
      } // bottom
    ...
    this._emit(enumEvents.RENDER, [coord]);
  ...
  getContent() {
    ...
    return ((_this$contLines = this.contLines) === null || _this$contLines === void 0 ? void 0 : _this$contLines.fake.join(enumControlChars.LF)) ?? '';
  }
  ...
  _align(line, width, align) {
    ...
    if (s < 0) return line;
    if (align === enumCoordInfos.CENTER && (s = Array((s / 2 | 0) + 1).join(' '))) return s + line + s;else if (align === enumCoordInfos.RIGHT && (s = Array(s + 1).join(' '))) return s + line;else if (this.parseTags && ~line.indexOf('{|}')) {
  ...
  enableDrag(verify) {
    ...
    if (typeof verify !== enumDataTypes.FUN) verify = () => true;
    this.enableMouse();
  ...
  setLabel(options) {
    ...
    if (typeof options === enumDataTypes.STR) options = {
      text: options
    };
    ...
    if (this._label) {
      ...
      if (options.side !== enumCoordInfos.RIGHT) {
    ...
    this.on(enumEvents.SCROLL, this._labelScroll = () => reposition());
    this.on(enumEvents.RESIZE, this._labelResize = () => helpers.nextTick(() => reposition()));
  }
  ...
  insertLine(i, line) {
    if (typeof line === enumDataTypes.STR) line = line.split(enumControlChars.LF);
    ...
    if (i >= this.contLines.ftor.length) {
      real = vectorIndex.last(this.contLines.ftor);
      real = vectorIndex.last(real) + 1;
    } else {
    ...
    this.setContent(this.contLines.fake.join(enumControlChars.LF), true);
  ...
  setLine(i, line) {
    ...
    return this.setContent(this.contLines.fake.join(enumControlChars.LF), true);
  }
  ...
  strWidth(text) {
    ...
    return this.screen.fullUnicode ? unicode__namespace.strWidth(text) : helpers.dropUnicode(text).length;
  }
...
class Scroll {
  ...
  config(options) {
    ...
    if (this.scrollbar) {
      ...
      this.style.scrollbar = this.style.scrollbar ?? this.scrollbar.style ?? objectSelect.select.call(SGR_ATTRS, this.scrollbar);

      if (this.track || this.scrollbar.track) {
        this.track = this.scrollbar.track || this.track;
        this.style.track = this.style.scrollbar.track || this.style.track;
        this.track.ch = this.track.ch || ' ';
        this.style.track = this.style.track ?? this.track.style ?? objectSelect.select.call(SGR_ATTRS, this.track);
        this.track.style = this.style.track;
      } // Allow controlling of the scrollbar via the mouse:


      if (options.mouse) {
        this.on(enumEvents.MOUSEDOWN, data => {
          ...
          if (x === self.width - self.intR - 1) {
            ...
            self.onScreenEvent(enumEvents.MOUSEDOWN, smd = data => {
              const y = data.y - self.absT;
            ...
            self.onScreenEvent(enumEvents.MOUSEUP, smu = () => {
              self._scrollingBar = false;
              self.removeScreenEvent(enumEvents.MOUSEDOWN, smd);
              self.removeScreenEvent(enumEvents.MOUSEUP, smu);
            });
          }
        });
      }
    ...
    if (options.mouse) {
      this.on(enumEvents.WHEELDOWN, () => {
        self.scroll(self.height / 2 | 0 || 1);
        self.screen.render();
      });
      this.on(enumEvents.WHEELUP, () => {
        self.scroll(-(self.height / 2 | 0) || -1);
        self.screen.render();
      });
    }
    ...
    if (options.keys && !options.ignoreKeys) {
      this.on(enumEvents.KEYPRESS, (ch, key) => {
        ...
        if (name === enumKeyNames.UP || vi && name === 'k') return void (self.scroll(-1), self.screen.render());
        if (name === enumKeyNames.DOWN || vi && name === 'j') return void (self.scroll(1), self.screen.render());
    ...
    this.on(enumEvents.PARSED_CONTENT, () => self._recalculateIndex());
  ...
  scroll(offset, always) {
    ...
    if (this.subBase === base) return this.emit(enumEvents.SCROLL); // When scrolling text, we want to be able to handle SGR codes as well as line
...
class Box extends Element {
  ...
  constructor(options = {}) {
    ...
    if (options.scrollable) {
      ...
      mixin__namespace.assign(this, Scroll.prototype);
...
class Log extends ScrollableText {
  ...
  constructor(options = {}) {
    ...
    this.on(enumEvents.SET_CONTENT, () => {
      if (!self._userScrolled || self.scrollOnInput) helpers.nextTick(() => {
  ...
  add(...args) {
    if (typeof args[0] === enumDataTypes.OBJ) {
...
class Screen extends componentsNode.Node {
  ...
  constructor(options = {}) {
    ...
    super(options, true);
    globalScreen.GlobalScreen.initialize(this);
    this.setupProgram(options);
    componentsNode.Node.prototype.setup.call(this, options); // super(options) // Node.call(this, options)
  ...
  config(options) {
    ...
    this.program.on(enumEvents.RESIZE, () => {
      self.alloc();
      self.render();

      function resizeEmitter(node) {
        node.emit(enumEvents.RESIZE);
        node.sub.forEach(resizeEmitter);
      }

      resizeEmitter(self);
    });
    this.program.on(enumEvents.FOCUS, () => self.emit(enumEvents.FOCUS));
    this.program.on(enumEvents.BLUR, () => self.emit(enumEvents.BLUR));
    this.program.on(enumEvents.WARNING, text => self.emit(enumEvents.WARNING, text));
    this.on(enumEvents.NEW_LISTENER, type => {
      const pressListen = type === enumEvents.KEYPRESS || type.indexOf('key ') === 0;
      const mouseListen = type === enumEvents.MOUSE;
      ...
      if (type === enumEvents.MOUSE || type === enumEvents.CLICK || type === enumEvents.MOUSEOVER || type === enumEvents.MOUSEOUT || type === enumEvents.MOUSEDOWN || type === enumEvents.MOUSEUP || type === enumEvents.MOUSEWHEEL || type === enumEvents.WHEELDOWN || type === enumEvents.WHEELUP || type === enumEvents.MOUSEMOVE) self._listenMouse();
    });
    this.setMaxListeners(Infinity);
    this.enter();
    this.postEnter();
    this.on('adjourn', () => globalScreen.GlobalScreen.journal = false);
  }
  ...
  alloc(dirty) {
    ...
    for (let y = 0, normCell = utilMorisot.Mor.build(this.dattr, enumChars.SP); y < this.rows; y++) {
  ...
  fillRegion(at, ch, xLo, xHi, yLo, yHi, override) {
    ...
    for (let line, temp = utilMorisot.Mor.build(at, ch); yLo < yHi; yLo++) {
  ...
  #reduceColor(color) {
    return utilByteColors.degrade(color, this.tput.colors);
  }
  ...
  #cursorAttr(cursor, normAttr) {
    ...
    if (shape === 'line') {
      ...
      at |= 7 << 9;
      at |= 8 << 18;
    } else if (typeof shape === enumDataTypes.OBJ && shape) {
      cursorAttr = utilSgrAttr.styleToAttr(cursor, shape);
    ...
    return utilMorisot.Mor.build(at, ch);
  }
  ...
  screenshot(xLo, xHi, yLo, yHi, term) {
    ...
    for (let y = yLo, line; y < yHi; y++) {
      ...
      for (let x = xLo, cell; x < xHi; x++) {
        ...
        if (at !== currAttr) {
          if (currAttr !== normAttr) {
            out += enumControlChars.CSI + enumCsiCodes.SGR;
          }
          ...
          if (at !== normAttr) {
            ...
            out += utilSgrAttr.attrToSgra(nextAttr, this.tput.colors);
          }
        }

        if (this.fullUnicode && unicode__namespace.charWidth(cell.ch) === 2) {
          x === xHi - 1 ? ch = ' ' : x++;
        }
      ...
      if (out) {
        main += (y > 0 ? enumControlChars.LF : '') + out;
      }
    }

    main = main.replace(/(?:\s*\x1b\[40m\s*\x1b\[m\s*)*$/, '') + enumControlChars.LF;
  ...
  attrCode(sgra, baseAttr, normAttr) {
    return utilSgrAttr.sgraToAttr(sgra, baseAttr, normAttr);
  } // sgra to blessed attr


  codeAttr(attr) {
    return utilSgrAttr.attrToSgra(attr, this.tput.colors);
      ...
      for (let x = 0, currCell, prevCell; x < currLine.length && (currCell = currLine[x]); x++) {
        ...
        if (options.useBCE && ch === ' ' && (tput.booleans.back_color_erase || (at & 0x1ff) === (this.dattr & 0x1ff)) && (at >> 18 & 8) === (this.dattr >> 18 & 8)) {
          ...
          if (clr && neq) {
            ...
            if (at !== currAttr) {
              out += utilSgrAttr.attrToSgra(at, this.tput.colors), currAttr = at;
            }
        ...
        if (at !== currAttr) {
          ...
          if (at !== this.dattr) {
            ...
            out += utilSgrAttr.attrToSgra(at, this.tput.colors);
          }
        ...
        if (this.fullUnicode) {
          ...
          if (unicode__namespace.charWidth(currLine[x].ch) === 2) {
            ...
            if (x === currLine.length - 1 || enumAngleTable.ANGLES[currLine[x + 1].ch]) {
  ...
  blankLine(ch, dirty) {
    const out = [];
    const tempCell = utilMorisot.Mor.build(this.dattr, ch || ' ');
  ...
  #dockBorders() {
    ...
    for (let i = 0, y, line, cell; i < stops.length; i++) {
      ...
      for (let x = 0; x < this.width; x++) {
        ...
        if (enumAngleTable.ANGLES[cell.ch]) {
  ...
  enter() {
    ...
    if (process.platform === 'win32') try {
      cp__default["default"].execSync('cls', {
        stdio: 'ignore',
        timeout: 1000
      });
    } catch (e) {}
  ...
  postEnter() {
    ...
    if (this.options.warnings) {
      this.on(enumEvents.WARNING, text => {
  ...
  _destroy() {
    this.leave();
    const index = globalScreen.GlobalScreen.instances.indexOf(this);

    if (~index) {
      globalScreen.GlobalScreen.removeInstanceAt(index); // GlobalScreen.instances.splice(index, 1)
      // GlobalScreen.total--
  ...
  _listenMouse(el) {
    ...
    this.program.on(enumEvents.MOUSE, function (data) {
      ...
      if (self._needsClickableSort) {
        self.clickable = helpers__namespace.hsort(self.clickable);
      ...
      for (let i = 0, node, pos; i < self.clickable.length && (node = self.clickable[i]); i++) {
        ...
        if (pos.xLo <= data.x && data.x < pos.xHi && pos.yLo <= data.y && data.y < pos.yHi) {
          node.emit(enumEvents.MOUSE, data);

          if (data.action === enumEvents.MOUSEDOWN) {
            self.mouseDown = node;
          } else if (data.action === enumEvents.MOUSEUP) {
            (self.mouseDown || node).emit(enumEvents.CLICK, data);
            self.mouseDown = null;
            ...
            if (self.hover !== node && !set) {
              ...
              node.emit(enumEvents.MOUSEOVER, data);
              self.hover = node;
            }
      ...
      if ((data.action === enumEvents.MOUSEMOVE || data.action === enumEvents.MOUSEDOWN || data.action === enumEvents.MOUSEUP) && self.hover && !set) {
        self.hover.emit(enumEvents.MOUSEOUT, data);
        self.hover = null;
      }
    ...
    this.on(enumEvents.ELEMENT_CLICK, el => {
  ...
  deleteLineNC(n, y, top, bottom) {
    ...
    this._buf += Array(n + 1).join(enumControlChars.LF);
    this._buf += this.tput.csr(0, this.height - 1);
    const j = bottom + 1;
  ...
  #getAngle(lines, x, y) {
    ...
    if (lines[y][x - 1] && enumAngleTable.ANGLES_L[lines[y][x - 1].ch]) {
    ...
    if (lines[y - 1] && enumAngleTable.ANGLES_U[lines[y - 1][x].ch]) {
      if (!this.options.ignoreDockContrast) {
    ...
    if (lines[y][x + 1] && enumAngleTable.ANGLES_R[lines[y][x + 1].ch]) {
      if (!this.options.ignoreDockContrast) {
        if (lines[y][x + 1][0] !== attr) return ch;
      }
    ...
    if (lines[y + 1] && enumAngleTable.ANGLES_D[lines[y + 1][x].ch]) {
      if (!this.options.ignoreDockContrast) {
        if (lines[y + 1][x][0] !== attr) return ch;
      }
    ...
    return enumAngleTable.ANGLE_TABLE[angle] || ch;
  }
  ...
  exec(file, args, options, callback) {
    ...
    ps.on(enumEvents.ERROR, err => callback ? callback(err, false) : void 0);
    ps.on(enumEvents.EXIT, code => callback ? callback(null, code === 0) : void 0);
    return ps;
  }

  readEditor(options, callback) {
    if (typeof options === enumDataTypes.STR) {
      options = {
        editor: options
      };
    }
  ...
  displayImage(file, callback) {
    ...
    ps.on(enumEvents.ERROR, err => callback ? callback(err) : void 0);
    ps.on(enumEvents.EXIT, code => callback ? code !== 0 ? callback(new Error('Exit Code: ' + code)) : callback(null, code === 0) : void 0);
    ps.stdin.write(input + enumControlChars.LF);
    ps.stdin.end();
  }
  ...
  setEffects(el, fel, over, out, effects, temp) {
    ...
    if (typeof el !== enumDataTypes.FUN) {
      const _el = el;
    ...
    fel.on(over, function () {
      ...
      Object.keys(effects).forEach(key => {
        ...
        if (val !== null && typeof val === enumDataTypes.OBJ) {
...
class Terminal extends Box {
  ...
  bootstrap() {
    ...
    this.on(enumEvents.FOCUS, () => {
      self.term.focus();
    });
    this.on(enumEvents.BLUR, () => {
      self.term.blur();
    });
    this.term.on(enumEvents.TITLE, title => {
      self.title = title, self.emit(enumEvents.TITLE, title);
    });
    this.term.on(enumEvents.PASSTHROUGH, data => {
      self.screen.program.flush(), self.screen.program.write(data);
    });
    this.on(enumEvents.RESIZE, () => helpers.nextTick(() => self.term.resize(self.width - self.intW, self.height - self.intH)));
    this.once(enumEvents.RENDER, () => self.term.resize(self.width - self.intW, self.height - self.intH));
    this.on(enumEvents.DESTROY, () => {
      self.kill(), self.screen.program.input.removeListener(enumEvents.DATA, self._onData);
    });
    ...
    this.on(enumEvents.RESIZE, () => helpers.nextTick(() => {
      try {
    ...
    this.onScreenEvent(enumEvents.KEYPRESS, () => self.screen.render());
...
Object.defineProperty(exports, 'Node', {
  enumerable: true,
  get: function () { return componentsNode.Node; }
});
exports.Box = Box;
exports.Cadre = Cadre;
exports.Coord = Coord;
