...
class GpmClient extends events.EventEmitter {
  constructor(options = {}) {
    ...
    if (tty) {
      fs__default["default"].stat(GPM_SOCKET, function (err, stat) {
        ...
        this.gpm = gpm;
        gpm.on(enumEvents.CONNECT, () => {
          send_config(gpm, conf, () => {
            conf.pid = 0;
            conf.vc = GPM_REQ_NOPASTE; //send_config(gpm, conf);
          });
        });
        gpm.on(enumEvents.DATA, packet => {
          const event = parseEvent(packet);

          switch (event.type & 15) {
            case GPM_MOVE:
              if (event.dx || event.dy) {
                self.emit(enumEvents.MOVE, event.buttons, event.modifiers, event.x, event.y);
              }

              if (event.wdx || event.wdy) {
                self.emit(enumEvents.MOUSEWHEEL, event.buttons, event.modifiers, event.x, event.y, event.wdx, event.wdy);
              }
            ...
            case GPM_DRAG:
              if (event.dx || event.dy) {
                self.emit(enumEvents.DRAG, event.buttons, event.modifiers, event.x, event.y);
              }
            ...
            case GPM_DOWN:
              self.emit(enumEvents.BTNDOWN, event.buttons, event.modifiers, event.x, event.y);

              if (event.type & GPM_DOUBLE) {
                self.emit(enumEvents.DBLCLICK, event.buttons, event.modifiers, event.x, event.y);
              }

              break;

            case GPM_UP:
              self.emit(enumEvents.BTNUP, event.buttons, event.modifiers, event.x, event.y);

              if (!(event.type & GPM_MFLAG)) {
                self.emit(enumEvents.CLICK, event.buttons, event.modifiers, event.x, event.y);
              }

              break;
          }
        });
        gpm.on(enumEvents.ERROR, () => self.stop());
      });
    }
  }
  ...
  buttonName(btn) {
    return btn & 4 ? enumKeyNames.LEFT : btn & 2 ? enumKeyNames.MIDDLE : btn & 1 ? enumKeyNames.RIGHT : '';
  }
