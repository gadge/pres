...
var utilBlessedColors = require('@pres/util-blessed-colors');
var enumDataTypes = require('@typen/enum-data-types');

class LCD extends componentsLayout.Canvas {
  constructor(options = {}) {
    ...
    options.elementPadds = options.padding || 2; // how far away from the edges to put the elements
    //coloring
...
class SixteenSegment {
  constructor(count, canvas, width, height, x, y, options) {
    ...
    this.Padding = options.elementPadds; // Padding around the display
  ...
  setOptions(options) {
    ...
    this.Padding = options.elementPadds || this.Padding;
...
class ProgressBar extends componentsForm.Input {
  constructor(options = {}) {
    ...
    if (typeof this.filled === enumDataTypes.STR) this.filled = +this.filled.slice(0, -1);
  ...
  render() {
    ...
    dattr = utilBlessedColors.styleToAttr(this.style.bar);
