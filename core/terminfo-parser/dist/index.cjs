...
var url = require('url');
var enumTerminfoAlias = require('@pres/enum-terminfo-alias');
...
class TerminfoLib {
  ...
  static initialize() {
    ...
    for (const {
      rows
    } of [enumTerminfoAlias.BooleanCapabilities, enumTerminfoAlias.NumberCapabilities, enumTerminfoAlias.StringCapabilities]) {
...
class TerminfoParser {
  ...
  #useInternalInfo(name) {
    return this.injectTerminfo(USR + path__default["default"].basename(name));
  }

  #useInternalCap(name) {
    return this.injectTermcap(USR + path__default["default"].basename(name) + '.termcap');
  }

  _prefix(term) {
    // If we have a terminfoFile, or our term looks like a filename, use it.
    if (term) {
      if (~term.indexOf(path__default["default"].sep)) return term;
  ...
  parseTerminfo(data, file) {
    ...
    utilHelpers.LogService.localInfo("terminfo-parser-info", info); // Extended Header
    ...
    if (this.extended !== false) {
      ...
      if (i < l - 1) {
        ...
        SCOPES.forEach(key => utilHelpers.merge(info[key], extended[key]));
      }
    }
  ...
  compile(info) {
    ...
    for (const key of TerminfoLib.booleans) if (nullish.nullish(methods[key])) methods[key] = false;

    for (const key of TerminfoLib.numerics) if (nullish.nullish(methods[key])) methods[key] = -1;

    for (const key of TerminfoLib.literals) if (nullish.nullish(methods[key])) methods[key] = noop;
  ...
  inject(info) {
    ...
    for (const [key, method] of Object.entries(info.methods || info)) this[key] = typeof method === enumDataTypes.FUN ? function () {
      return method.call(self, utilHelpers.slice(arguments));
    } : method;
  ...
  #compile(info, key, value) {
    ...
    if (type === enumDataTypes.BOO || type === enumDataTypes.NUM) return value;
    if (!value || type !== enumDataTypes.STR) return noop; // See:
    // ~/ncurses/progs/tput.c - tput() - L149
    // ~/ncurses/progs/tset.c - set_init() - L992

    if (key === 'init_file' || key === 'reset_file') {
      try {
        value = fs__default["default"].readFileSync(value, 'utf8');
  ...
  translateTermcap(info) {
    ...
    SCOPES.forEach(key => {
      ...
      Object.keys(source).forEach(cap => {
        if (key === enumDataTypes.STR) info.literals[cap] = capToInfo.call(self, cap, info.literals[cap], 1);
  ...
  has(name) {
    ...
    return !name ? false : typeof val === enumDataTypes.NUM ? val !== -1 : !!val;
  }
      ...
      if (term && term.literals.tc) {
        ...
        if (inherit) {
          SCOPES.forEach(function (type) {
            utilHelpers.merge(term[type], inherit[type]);
          });
        }
      }
...
exports.TerminfoLib = TerminfoLib;
exports.TerminfoParser = TerminfoParser;
exports.sprintf = sprintf;
exports.tryRead = tryRead;
exports.whichTerminal = whichTerminal;
