...
import { STR, FUN, NUM, BOO } from '@typen/enum-data-types';
...
noop.unsupported = true; // export function merge(a, b) { return Object.keys(b).forEach(key => a[key] = b[key]), a }
...
function termPrint(code, print, done) {
...
TputDes.ipaths = IPATHS;
TputDes.cpaths = CPATHS;
TputDes._prefix = CPATHS;
TputDes._tprefix = CPATHS;
TputDes.alias = {};
TputDes.keyMap = {};
TputDes.termcap = TERMCAP;
TputDes.boo = CAPS_BOO;
TputDes.num = CAPS_NUM;
TputDes.str = CAPS_STR;
TputDes.acsc = ACSC;
TputDes.utoa = UTOA;
TputDes.initialize();
...
var _debug = /*#__PURE__*/_classPrivateFieldLooseKey("debug");

var _useVt102Cap = /*#__PURE__*/_classPrivateFieldLooseKey("useVt102Cap");

var _useXtermCap = /*#__PURE__*/_classPrivateFieldLooseKey("useXtermCap");

var _useXtermInfo = /*#__PURE__*/_classPrivateFieldLooseKey("useXtermInfo");

var _useInternalInfo = /*#__PURE__*/_classPrivateFieldLooseKey("useInternalInfo");

var _useInternalCap = /*#__PURE__*/_classPrivateFieldLooseKey("useInternalCap");

var _compile = /*#__PURE__*/_classPrivateFieldLooseKey("compile");
...
class Tput {
  ...
  constructor(options = {}) {
    ...
    Object.defineProperty(this, _useInternalCap, {
      value: _useInternalCap2
    });
    Object.defineProperty(this, _useInternalInfo, {
      value: _useInternalInfo2
    });
    Object.defineProperty(this, _useXtermInfo, {
      value: _useXtermInfo2
    });
    Object.defineProperty(this, _useXtermCap, {
      value: _useXtermCap2
    });
    Object.defineProperty(this, _useVt102Cap, {
      value: _useVt102Cap2
    });
    ...
    this.error = null;
    this.utoa = UTOA;
    this._readTermcap = this.readTermcap;
    if (typeof options === STR) options = {
      terminal: options
    };
    this.configTput(options);
    if (options.terminal || options.term) this.setup();
    console.log('>> [new tput]', `[boo] (${Object.keys(this.boo).length})`, `[num] (${Object.keys(this.num).length})`, `[str] (${Object.keys(this.str).length})`, `[all] (${Object.keys(this.all).length})`, `[colors] (${this.colors})`);
  }
    ...
    try {
      if (this.termcap) {
        try {
          ...
          _classPrivateFieldLooseBase(this, _useInternalCap)[_useInternalCap](this.terminal);
        }
      } else {
        try {
          this.injectTerminfo();
          ...
          _classPrivateFieldLooseBase(this, _useInternalInfo)[_useInternalInfo](this.terminal);
        }
      }
    } catch (e) {
      ...
      _classPrivateFieldLooseBase(this, _useXtermInfo)[_useXtermInfo]();
    }
  }
  ...
  parseTerminfo(data, file) {
    ...
    Object.keys(info.str).forEach(key => {
      if (info.str[key] === -1) return void delete info.str[key]; // Workaround: fix an odd bug in the screen-256color terminfo where it tries
  ...
  parseExtended(data) {
    ...
    for (; i < l; i += 2) data[i + 1] === 0xff && data[i] === 0xff ? _numbers.push(-1) : _numbers.push(data[i + 1] << 8 | data[i]); // Strings Section


    const _strings = [];
    l = i + h.strCount * 2;

    for (; i < l; i += 2) data[i + 1] === 0xff && data[i] === 0xff ? _strings.push(-1) : _strings.push(data[i + 1] << 8 | data[i]); // Pass over the sym offsets and get to the string table.
    ...
    _strings.forEach(string => info.str[sym[j++]] = string); // Should be the very last bit of data.
  ...
  compile(info) {
    ...
    _classPrivateFieldLooseBase(this, _debug)[_debug](info);

    const all = info.all = {};
    const methods = info.methods = {}; // boo, num, str

    for (const scope of SCOPES) for (const [key, value] of Object.entries(info[scope])) {
      methods[key] = _classPrivateFieldLooseBase(this, _compile)[_compile](info, key, all[key] = value);
    }

    for (const key of TputDes.boo) if (nullish(methods[key])) methods[key] = false;

    for (const key of TputDes.num) if (nullish(methods[key])) methods[key] = -1;

    for (const key of TputDes.str) if (nullish(methods[key])) methods[key] = noop;

    for (const [key, method] of Object.entries(methods)) if (TputDes.alias[key]) for (const alias of TputDes.alias[key]) {
  ...
  _captoinfo(cap, s, parameterized) {
    ...
    function warn() {
      ...
      return _classPrivateFieldLooseBase(self, _debug)[_debug].apply(self, args);
    }
  ...
  detectBrokenACS(info) {
    ...
    if (((_ref = info.num || info.numbers) == null ? void 0 : _ref.U8) >= 0) return !!(info.num || info.numbers).U8; // The linux console is just broken for some reason.
    ...
    if (this.termcap && info.name.indexOf('screen') === 0 && process.env.TERMCAP && ~process.env.TERMCAP.indexOf('screen') && ~process.env.TERMCAP.indexOf('hhII00')) {
      if (~info.str.enter_alt_charset_mode.indexOf('\x0e') || ~info.str.enter_alt_charset_mode.indexOf('\x0f') || ~info.str.set_attributes.indexOf('\x0e') || ~info.str.set_attributes.indexOf('\x0f')) {
        return true;
      }
    }
  ...
  readTermcap(term) {
    ...
    if (this.debug) this._termcap = terms(function tc(term) {
      if (term && term.str.tc) {
        ...
        const names = terms[term.str.tc] ? terms[term.str.tc].names : [term.str.tc];

        _classPrivateFieldLooseBase(self, _debug)[_debug]('%s inherits from %s.', term.names.join('/'), names.join('/'));
  ...
  readTerminfo(term) {
    ...
    if (this.debug) this._terminfo = info; // console.log('>> [tput.readTerminfo]', term, 'max_colors', info.num.max_colors)

    if (term.endsWith('xterm')) info.num.max_colors = 256;
    return info;
  }
...
function _useVt102Cap2() {
  return this.injectTermcap('vt102');
}

function _useXtermCap2() {
  return this.injectTermcap(USR + 'xterm.termcap');
}

function _useXtermInfo2() {
  return this.injectTerminfo(USR + 'xterm');
}

function _useInternalInfo2(name) {
  return this.injectTerminfo(USR + path.basename(name));
}

function _useInternalCap2(name) {
  return this.injectTermcap(USR + path.basename(name) + '.termcap');
}

function _compile2(info, key, value) {
  let v;
  if (key === 'max_colors') console.log('>> [tput.#compile]', key, value);

  _classPrivateFieldLooseBase(this, _debug)[_debug]('Compiling %s: %s', key, JSON.stringify(value));
  ...
  while (val) {
    ...
    if (read(/^\^(.)/i, true)) {
      if (!(ch >= ' ' && ch <= '~')) {
        _classPrivateFieldLooseBase(this, _debug)[_debug]('%s: bad caret char.', tkey); // NOTE: ncurses appears to simply
    ...
    if (read(/^\\([eEnlrtbfs\^\\,:0]|.)/, true)) {
      ch = ch === 'e' || ch === 'E' ? '\x1b' : ch === 'n' ? '\n' : ch === 'l' ? '\x85' : ch === 'r' ? '\r' : ch === 't' ? '\t' : ch === 'b' ? '\x08' : ch === 'f' ? '\x0c' : ch === 's' ? ' ' : ch === '^' ? '^' : ch === '\\' ? '\\' : ch === ',' ? ',' : ch === ':' ? ':' : ch === '0' ? '\x80' : ch === 'a' ? '\x07' : (_classPrivateFieldLooseBase(this, _debug)[_debug]('%s: bad backslash char.', tkey), cap[0]);
      print(ch);
      continue;
    } // $<5> -> padding
...
export { Tput, TputDes, sprintf, tryRead };
