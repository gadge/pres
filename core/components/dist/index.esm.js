...
import { Box, Element, Screen, Layout, Line, Log, ScrollableBox, ScrollableText, Terminal } from '@pres/components-core';
import { DataTable, List, ListBar, ListTable, Table, Tree } from '@pres/components-data';
import { Button, Checkbox, FileManager, Form, Input, Prompt, Question, RadioButton, RadioSet, Textbox, Textarea } from '@pres/components-form';
import { Map } from '@pres/components-geo';
import { LCD, Loading, Message, ProgressBar } from '@pres/components-inform';
import { Canvas, Carousel, Grid } from '@pres/components-layout';
import { LogList, Markdown, Text } from '@pres/components-text';
import { ANSIImage, BigText, Image, OverlayImage, Picture, Video } from '@pres/components-visual';
...
class Pres {
  ...
  static scrollableBox(options) {
    return new ScrollableBox(options);
  }

  static scrollableText(options) {
    return new ScrollableText(options);
  }

  static terminal(options) {
  ...
  static listBar(options) {
    return new ListBar(options);
  }

  static listTable(options) {
  ...
  static radioButton(options) {
    return new RadioButton(options);
  }

  static radioSet(options) {
    return new RadioSet(options);
  }

  static textbox(options) {
  ...
  static canvas(options, canvasType) {
    return new Canvas(options, canvasType);
  }

  static carousel(pages, options) {
  ...
  static ansiImage(options) {
    return new ANSIImage(options);
  }

  static bigText(options) {
