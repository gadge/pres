...
import { NUM, STR, DEF } from '@typen/enum-data-types';
...
class Mor extends Array {
  ...
  toSgr(total) {
    return attrToSgra(this.at, total);
  }

  render(text) {
    return this.toSgr() + text + CSI + SGR;
  }

  toString() {
    return this.modeSign + String(this.fore).padStart(3, '0') + DOT + String(this.back).padStart(3, '0');
  }
