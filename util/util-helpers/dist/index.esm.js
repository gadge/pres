...
const dropUnicode = text => !text ? '' : text.replace(unicode.chars.all, '??').replace(unicode.chars.combining, '').replace(unicode.chars.surrogate, '?'); // const parseTags = function (text, screen) {
...
export { MergeRecursive, abbrNumber, asort, MergeRecursive as assignDeep, cleanTags, dropUnicode, escape, findFile, generateTags, getColorCode, getTypeName, hsort, merge, slice, stripTags };
