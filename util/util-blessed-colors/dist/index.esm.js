...
Manet.RGB_COLORS = [];
Manet.SPARSE_NAMES = [];
Manet.COLOR_MAPPING = LOCAL_COLOR_MAPPING;
...
function convert(color) {
  color = typeof color === NUM ? color : typeof color === STR ? COLOR_NAMES[color = color.replace(/[\- ]/g, '')] ?? match(color) : Array.isArray(color) ? match(color) : -1;
...
function match(...args) {
  const [ini] = args;
  args = typeof ini === STR ? ini[0] !== '#' ? null : hexToRGB(ini) : Array.isArray(ini) ? ini : args;
...
function sattr(style, fg, bg) {
  ...
  if (typeof bg === FUN) bg = bg(this); // console.log('>> [element.sattr]', this.codename, fg ?? AEU, 'to', colors.convert(fg), bg ?? AEU, 'to', colors.convert(bg))
...
export { BLEND_CACHE, COLOR_MAPPING, COLOR_NAMES, HEX_COLORS, RGBToHex, RGB_COLORS, SPARSE_NAMES, XTERM_COLORS, attrToSgra, blend, COLOR_MAPPING as ccolors, HEX_COLORS as colors, convert, reduce as degrade, hexToRGB, match, mixColors, SPARSE_NAMES as ncolors, reduce, sattr, sgraToAttr, sattr as styleToAttr, match as toByte, RGB_COLORS as vcolors };
