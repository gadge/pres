...
function charWidth(str, i) {
  ...
  if (point === 0x09) {
    var _GlobalScreen$global

    return ( ( _GlobalScreen$global = globalScreen.GlobalScreen.global ) === null || _GlobalScreen$global === void 0 ? void 0 : _GlobalScreen$global.tabc.length ) ?? 8
  } // 8-bit control characters (2-width according to unicode??)


  if (point < 32 || point >= 0x7f && point < 0xa0) return 0 // search table of non-spacing characters
  // is ucs combining or C0/C1 control character

  if (combining[point]) return 0 // check for double-wide
  // if (point >= 0x1100
