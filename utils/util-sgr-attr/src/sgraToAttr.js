...
import { rgbToByte }                             from '@pres/util-byte-colors'
...
export function sgraToAttr(sgra, baseAttr, normAttr) {
  ...
  for (let i = 0, hi = ve.length, ch, nx; i < hi; i++) {
    ...
    fore = 30 <= ch && ch <= 37 ? ch - 30
      : ch === 38 && ( nx = +ve[++i] ) ? ( nx === 5 ? +ve[++i] : nx === 2 ? rgbToByte(+ve[++i], +ve[++i], +ve[++i]) ?? normAttr >> 9 & 0x1ff : fore )
        : ch === 39 ? normAttr >> 9 & 0x1ff
          : 90 <= ch && ch <= 97 ? ( fore = ch - 90, fore += 8 )
            : fore
    back = 40 <= ch && ch <= 47 ? ch - 40
      : ch === 48 && ( nx = +ve[++i] ) ? ( nx === 5 ? +ve[++i] : nx === 2 ? rgbToByte(+ve[++i], +ve[++i], +ve[++i]) ?? normAttr & 0x1ff : back )
