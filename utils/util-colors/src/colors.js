...
import { COLOR_MAPPING, COLOR_NAMES } from '../assets'
import { RGB_COLORS, SPARSE_NAMES }   from './manet'
...
export function convert(color) {
  color = typeof color === NUM
    ? color
    : typeof color === STR && (color = color.replace(/[\- ]/g, '')) ? COLOR_NAMES[color] ?? match(color)
      : Array.isArray(color)
        ? match(color)
        : -1
